...
Twitter.prototype = {
  ...
  updateWithMedia: function(params, callback) {
		var crlf = "\r\n";
		var boundary = '10102754414578508781458777923';

		var separator = '--' + boundary;
		var footer = crlf + separator + '--' + crlf;
		var fileHeader = 'Content-Disposition: form-data; name="media[]"; filename="fotka.jpg"';

		var contents = '';
		Object.keys(params).forEach(function (key) {
			if (key !== 'media') {
				contents += separator + crlf
		...
		var multipartBody = Buffer.concat([
			new Buffer(contents),
			new Buffer(params['media'].toString('base64')),
			new Buffer(footer)]);

		var request = new OARequest(this.auth,
										'POST',
										REST_ROOT+'statuses/update_with_media.json',
